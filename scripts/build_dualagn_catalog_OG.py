...
def load_tangos_bh_data(snap, hdf5_path):
    ...
    # CRITICAL FIX: Filter out BHs with invalid halo assignments
    print(f'Loaded {len(df)} BHs from HDF5')
    print(f'BHs with pynbody_haloid = 0: {(df["pynbody_haloid"] == 0).sum()}')
    print(f'BHs with pynbody_haloid < 0: {(df["pynbody_haloid"] < 0).sum()}')
    
    # Remove BHs with invalid halo IDs (0 or negative)
    valid_mask = (df['pynbody_haloid'] > 0) & (df['BH_id'] > 0)
    df_clean = df[valid_mask].copy()
    
    print(f'Removed {len(df) - len(df_clean)} BHs with invalid halo IDs')
    print(f'Remaining valid BHs: {len(df_clean)}')
...
def find_dual_agn_pairs(df, sim, t_now, t_prev, separation_threshold=30, lbol_threshold=1e43):
    ...
    # Compute and store Lbol and Ledd (assumes BH_mdot in Msun/yr)
    df['BH_Lbol'] = 0.1 * df['BH_mdot'] * msun_to_g / yr_to_s * c_cgs**2
    df['BH_Ledd'] = 1.26e38 * df['BH_mass']

    # Select AGN by Lbol
    agn_mask = df['BH_Lbol'] > lbol_threshold
    main_bhs = df[agn_mask].copy()

    pairs = []
    pair_count = 0
    triple_count = 0

    # build positions array once for speed
    pos_array = df[['BH_x', 'BH_y', 'BH_z']].values

    for _, bh in main_bhs.iterrows():
        bh_pos = np.asarray([bh['BH_x'], bh['BH_y'], bh['BH_z']], dtype=float)
        deltas = pos_array - bh_pos
        distances = np.linalg.norm(deltas, axis=1)

        # require neighbor to also be AGN by Lbol and not the same BH
        neighbors = df[(distances < separation_threshold) & (df['BH_Lbol'] > lbol_threshold) & (df['BH_id'] != bh['BH_id'])]

        if len(neighbors) > 1:
            # multiple AGN nearby -> treat as multi system, skip here (or handle separately)
            triple_count += 1
            continue
        elif len(neighbors) == 1:
            neighbor = neighbors.iloc[0]

            # canonical ordering to avoid duplicated reversed pairs
            if bh['BH_id'] >= neighbor['BH_id']:
                continue
            ...
            neighbor_dict.update({
                ...
                'NeighbourBH_pynbody_haloid': neighbor.get('pynbody_haloid', np.nan)
            })
    ...
    print(f"Total AGN in snapshot (Lbol > {lbol_threshold:.2e}): {len(main_bhs)}  (unique BH_id: {main_bhs['BH_id'].nunique()})")
    print(f"Total dual AGN pairs found: {pair_count}")
    print(f"Triple+ systems skipped: {triple_count}")
...
def main(snapshot_index):
    ...
    snapshot_list = ['000098','000105','000111','000118','000126','000134','000142','000151','000160','000170','000181',\
            ...
            '008192']  
    #snapshot_list = ['001270']


    hdf5_path = "/scratch/stlock/halomap_files/HaloBH-TangosPynbodyMap-R25-snap{}.hdf5"
    output_path = "/scratch/stlock/dualAGNs/datasets/1e43lum/catalog_30kpc_v5/DualAGN-Catalog-R25-z{:.2f}.pkl"
