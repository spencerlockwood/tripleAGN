...
def load_snapshot(snapshot_index, snapshot_list, base_path):
    ...
    s.properties['snapshot'] = snap              # e.g. '000105'
    s.properties['snap_index'] = snapshot_index  # integer index
...
def find_triple_agn_systems(df, sim, t_now, t_prev, separation_threshold=30, mdot_threshold=1.89e-3, z=None):
    ...
    agn_bhs = df[df['BH_Lbol'] > 1e43].copy() # NEED TO CHANGE NEIGHBOR LBOL THRESHOLD TOO (BELOW)
    ...
    for idx, bh in agn_bhs.iterrows():
        ...
        # BH positions are now in physical kpc
        bh_pos = np.array([bh['BH_x'], bh['BH_y'], bh['BH_z']])
        
        # Calculate distances in physical kpc
        deltas = df[['BH_x', 'BH_y', 'BH_z']].values - bh_pos
        distances = np.linalg.norm(deltas, axis=1)

        # Select neighbor BHs within 15 PHYSICAL kpc
        ...
        if len(neighbors) == 2:
            ...
            # Compute separations in physical kpc
            ...
            # Compute local environments using physical positions
            ...
            triple_system['snapshot'] = sim.properties['snapshot']
            triple_system['snap_index'] = sim.properties['snap_index']
...
def compute_halo_environment_properties(sim, halo_ids, t_now, t_prev):
    ...
    for haloid in halo_ids_clean:
        try:
            ...
            sub25 = h[haloid][pnb.filt.Sphere(25, center_phys)]
            sub30 = h[haloid][pnb.filt.Sphere(30, center_phys)]
            sub50 = h[haloid][pnb.filt.Sphere(50, center_phys)]

            def half_mass_radius():
                # MassRadii[1][200.0] is physical
            ...
            # All outputs in physical kpc
            results.append({
                'Halo_id': int(haloid),
                'Halo_center_x': center_phys[0],
                'Halo_center_y': center_phys[1],
                'Halo_center_z': center_phys[2],
...
def main(snapshot_index):
    ...
    output_path = "/scratch/stlock/tripleAGNs/catalogs/1e43lum/catalogue_30kpc/TripleAGN-Catalog-R50-z{:.2f}.pkl"
    ...
    triple_agn_df = find_triple_agn_systems(matched_bh_df, s, t_now, t_prev, z)
